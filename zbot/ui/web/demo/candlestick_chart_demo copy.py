    ...
    try:
        if existing_data.empty:
            ...
            if not isinstance(last_date, datetime):
                print(
                    f"Warning: last_date is not a datetime object: {type(last_date)}")
                last_date = datetime.now() - timedelta(minutes=1)
        ...
        trend_changes = np.cumsum(
            [trend_strength * np.random.uniform(0.5, 1.5) for _ in range(new_points)])
        total_changes = base_changes + trend_changes

        new_closes = [last_close + change for change in total_changes]
        new_opens = [new_closes[i] - np.random.uniform(
            0, 0.5) if i > 0 else last_close for i in range(new_points)]
        new_highs = [max(new_opens[i], new_closes[i]) +
                     np.random.uniform(0, 1) for i in range(new_points)]
        new_lows = [min(new_opens[i], new_closes[i]) -
                    np.random.uniform(0, 1) for i in range(new_points)]
        ...
        combined_data = pd.concat([existing_data, new_data]).iloc[-100:]
    ...
    fac.Center([
        fac.Space([
            ...
            fac.Space([
                html.H1('实时蜡烛图播放器'),
                fac.Space([
                    fac.Button('播放', id='play-button', color='primary', variant='solid'),
                    fac.Button('暂停', id='pause-button', color='primary', variant='solid'),
                    fac.Button('停止', id='stop-button', color='primary', variant='solid'),
                ])
            ], direction='vertical'),

            fac.Space([
                html.Div(id='output-container', style={'width': '100%'}),
...
def update_chart(n_intervals, play_clicks, pause_clicks, stop_clicks,
    ...
    fig = fact.AntdStock(
        ...
        label={'content': 'text'}, # 标签，标注买入卖出信息
        
        # slider={'visible': True},
        # interactions=[{'enable': True, 'type': 'element-list-highlight', 'cfg': {
    ...
    # fig = create_candlestick_chart(df, title='价格')
    # # 创建蜡烛图
